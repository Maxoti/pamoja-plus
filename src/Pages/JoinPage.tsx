...
const executeJoin = async (
  ...
  // 1. Auth — create account or sign in if email already exists
  try {
  ...
  // 2. Create tenantMember document
  ...
  // 5. Mark invite as used — MUST be last critical step.
  //    Placing this after membership creation means a failure here is
  //    recoverable (admin can manually clean up), whereas placing it
  //    earlier would permanently burn the token before the user joins.
  ...
  // 6. Audit log — fire-and-forget. A log failure must never block a
  //    successful join. Matches the pattern used in Members.tsx.
...
export default function JoinPage() {
  ...
  // ── Token validation — READ ONLY, never mutates the invite document ─────────
  useEffect(() => {
    const validate = async () => {
      ...
      try {
        ...
        // Guard: past expiry — toDate() converts Firestore Timestamp correctly
        if (data.expiresAt.toDate() < new Date()) {
          setStep("expired");
          return;
        }

        // Load tenant display name for the welcome screen
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setError(msg);
      setStep("register"); // return user to form — token is still valid
    }
  };
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .group-badge {
          ...
          border-radius: 100px; padding: 6px 14px; margin-top: 16px;
          font-size: 13px; font-weight: 600; color: #E8A832;
        }
        .role-badge {
        ...
        .form-input {
          ...
          border: 1.5px solid #E8E8E0; border-radius: 11px;
        ...
        .form-input:disabled {
          background: #F5F4EF; color: #888; cursor: not-allowed;
        }
        ...
        .error-box {
          ...
          margin-bottom: 18px; font-size: 14px; color: #DC2626;
        }

        .btn-join {
          width: 100%; background: #1A3A2A; color: white;
        ...
        .state-center {
          ...
          padding: 64px 36px; text-align: center; gap: 12px;
        }
        ...
        .spinner {
          ...
          border: 3px solid #ECEAE3; border-top-color: #1A3A2A;
          border-radius: 50%;
        ...
        .already-have a {
          color: #1A3A2A; font-weight: 600; text-decoration: none;
        }
        ...
        {step === "register" && invite && (
          <>
            ...
            <div className="join-body">
              ...
              {/* Email — locked to invite, not editable */}
