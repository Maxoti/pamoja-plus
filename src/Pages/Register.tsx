...
const validators: Record<number, (f: FormData) => string> = {
  1: (f) => {
    if (!f.adminName.trim())              return "Full name is required";
    if (!f.adminEmail.trim())             return "Email is required";
    if (!f.adminPhone.trim())             return "Phone number is required";
  ...
  2: (f) => {
    if (!f.groupName.trim())    return "Group name is required";
    if (!f.groupLocation.trim()) return "Location is required";
    return "";
  },
};

const parseFriendlyError = (err: unknown): string => {
  if (!(err instanceof Error)) return "Something went wrong. Please try again.";
  const raw     = err.message;
  const matched = Object.entries(FRIENDLY_ERRORS).find(([key]) => raw.includes(key));
  return matched
    ? matched[1]
...
export default function Register() {
  const { registerWithEmail, applyRoleAfterRegistration } = useAuth();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // ── 2. Write tenant + user + member docs in parallel ───────────────────
      ...
      await Promise.all([
        ...
        setDoc(doc(db, "tenantMembers", `${tenantId}_${uid}`), {
          tenantId,
          userId:   uid,
          name:     form.adminName,
          email:    form.adminEmail,
      ...
      addDoc(collection(db, `tenants/${tenantId}/auditLogs`), {
        ...
        entityId:    tenantId,
        timestamp:   now,
      }).catch((err) => console.warn("[Register] audit log failed:", err));

      // ── 4. Hydrate role into context so dashboard renders correctly ─────────
      await applyRoleAfterRegistration(uid);

      navigate("/dashboard");

    } catch (err) {
      // ── Rollback: delete orphaned Auth user if Firestore writes failed ──────
      if (uid) {
        const { auth } = await import("../firebase");
        await auth.currentUser?.delete().catch(() =>
          console.error("[Register] rollback failed — orphaned Auth user:", uid)
        );
      }
  ...
  }, [form, registerWithEmail, applyRoleAfterRegistration, navigate]);
      ...
      <div className="reg-left">
        ...
        <div className="reg-testimonial">
          <div className="testimonial-text">
            "Pamoja Plus replaced our WhatsApp group and Excel sheets. Now every member can see
            their contribution history and the treasurer has full accountability."
          </div>
            ...
            <>
              ...
              <div className="form-group">
                ...
                <input className="form-input" autoComplete="name"
                  placeholder="e.g. Nyikwa Charles" value={form.adminName}
              ...
              <div className="form-group">
                ...
                <input className="form-input" type="email" autoComplete="email"
                  placeholder="you@example.com" value={form.adminEmail}
                  onChange={(e) => update("adminEmail", e.target.value)} />
              </div>

              <div className="form-group">
                <label className="form-label">Phone number (M-Pesa)</label>
                <input className="form-input" type="tel" autoComplete="tel"
                  placeholder="0712 345 678" value={form.adminPhone}
          ...
          {step === 2 && (
            <>
              ...
              <div className="form-group">
                ...
                <input className="form-input" placeholder="e.g. Kotuoma Welfare Group"
              ...
              <div className="form-group">
                ...
                <input className="form-input" placeholder="e.g. Nairobi, Kenya"
                  value={form.groupLocation}
                  onChange={(e) => update("groupLocation", e.target.value)} />
              </div>
              ...
              <div className="form-group">
                ...
                <textarea className="form-input" placeholder="What does your group do?"
...
// ── Styles (extracted from JSX for readability) ───────────────────────────────
