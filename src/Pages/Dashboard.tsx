...
const PILL: Record<string, { bg: string; fg: string }> = {
  ...
  fulfilled: { bg: "#ECFDF5", fg: "#059669" },
  closed:    { bg: "#F3F4F6", fg: "#6B7280" },
  active:    { bg: "#EFF6FF", fg: "#3B82F6" },
  none:      { bg: "#F3F4F6", fg: "#6B7280" },
};

const StatusPill = ({ status }: { status: string }) => {
  const p = PILL[status] ?? { bg: "#F3F4F6", fg: "#6B7280" };
  return (
    <span style={{
...
const Sidebar = ({
  currentPath, displayName, memberRole,
  ...
  const fetchDashboard = useCallback(async (uid: string) => {
    ...
    const [tenantSnap, contribSnap, annSnap, meetSnap, pledgeSnap, cycleSnap] =
      await Promise.all([
        ...
        getDocs(query(
          collection(db, `tenants/${tid}/contributions`),
          orderBy("createdAt", "desc"), limit(10),
        )),
        getDocs(query(
          collection(db, `tenants/${tid}/announcements`),
          orderBy("createdAt", "desc"), limit(5),
        )),
        getDocs(query(
          collection(db, `tenants/${tid}/meetings`),
          limit(5),
        )),
        getDocs(collection(db, `tenants/${tid}/pledges`)),
        getDocs(query(
          collection(db, `tenants/${tid}/cycles`),
    ...
    const fmtCycleDate = (ts: Timestamp) =>
      ts?.toDate().toLocaleDateString("en-KE", {
        day: "numeric", month: "short", year: "numeric",
      });
    ...
    setData({
      ...
      cycleMeta: cycle
        ? `${fmtCycleDate(cycle.startDate)} — ${fmtCycleDate(cycle.endDate)}`
        : "—",
    });
  }, []);
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", background: "#F5F4EF", display: "flex" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .contrib-table th { padding: 9px 22px; text-align: left; font-size: 10px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: #C0BAB0; background: #FAFAF7; border-bottom: 1px solid #F0EDE6; }
        .contrib-table td { padding: 13px 22px; font-size: 13.5px; color: #333; border-bottom: 1px solid #F7F5F0; vertical-align: middle; }
        .contrib-table tr:last-child td { border-bottom: none; }
        .contrib-table tbody tr { transition: background 0.12s; }
        .contrib-table tbody tr:hover td { background: #FAFAF7; }
        .mpesa-ref { font-family: 'Courier New', monospace; font-size: 11.5px; color: #888; background: #F5F4EF; padding: 2px 8px; border-radius: 5px; }
        .member-name { font-weight: 600; font-size: 13px; color: #1A1A1A; display: block; }
        .member-id { font-size: 11px; color: #BBB; font-family: 'Courier New', monospace; display: block; margin-top: 1px; }

        .ann-item { padding: 15px 22px; border-bottom: 1px solid #F0EDE6; transition: background 0.12s; cursor: default; }
            ...
            <div className="panel">
              ...
              {data.recentContributions.length === 0 ? (
                ...
                <table className="contrib-table">
                  <thead>
                    <tr>
                      <th>Member</th><th>Amount</th><th>M-Pesa Ref</th><th>Date</th><th>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.recentContributions.map((c) => (
                      <tr key={c.id}>
                        <td>
                          <span className="member-name">{c.userId}</span>
                          <span className="member-id">{c.id.slice(0, 12)}…</span>
                        </td>
